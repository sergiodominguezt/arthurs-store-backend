...
if ! pm2 describe arthurs-store-backend > /dev/null; then
  ...
  sudo pm2 start npm --name arthurs-store-backend -- run start:dev >> /home/ec2-user/arthurs-store-backend/deploy.log
else
  echo 'Restarting arthurs-store-backend' >> /home/ec2-user/arthurs-store-backend/deploy.log
  sudo pm2 restart arthurs-store-backend >> /home/ec2-user/arthurs-store-backend/deploy.log
fi
